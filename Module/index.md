...
[引用: module image](https://www.practical-go-lessons.com/img/3_modules.3b193265.png)
...
と書かれています。
[引用: package image](https://www.practical-go-lessons.com/img/go_program_package.acdaaa3b.png)
...
## 5. Go Module まとめ
- モジュールは、**パッケージの集合体**のこと
- モジュールの正式名称としてモジュールパスがあり、`go.mod`の"goディレクティブ"で宣言される
- `go.mod`および`go.sum`はモジュールに含まれず、モジュールを管理するためにあるファイルである
- `go.mod`はインポートパスとバージョン情報を書いておくためのファイル
  - バージョンを明示的に指定する必要がある場合がある
  - その場合はインポートする側でもバージョンの指定が必要
- `go.sum`は、`go.mod`を元に取得したモジュールが`go.sum`生成時のものと一致しているかをチェックするためのファイル
  - モジュールはより新しいバージョンを使おうとする
  - 最新バージョンを悪意ある第三者に置き換えられた場合などに検知できる

主に[Go Modules Reference - The Go Programming Language](https://go.dev/ref/mod#modules-overview)を元にmoduleとmodule周りの関係を調べました。
