...
- [8. テストとテスタビリティ](#8-テストとテスタビリティ)
  - [8-1. 単体テスト](#8-1-単体テスト)
      - [8-1-1. 単体テスト](#8-1-1-単体テスト)
      - [8-1-2. go test](#8-1-2-go-test)
      - [8-1-3. testingパッケージ](#8-1-3-testingパッケージ)
      - [8-1-4. テストの後処理](#8-1-4-テストの後処理)
      - [8-1-5. テスティングフレームを使わない理由](#8-1-5-テスティングフレームを使わない理由)
      - [8-1-6. Exampleテスト](#8-1-6-exampleテスト)
      - [8-1-7. テストの並列実行](#8-1-7-テストの並列実行)
      - [8-1-8. サブテスト](#8-1-8-サブテスト)
      - [8-1-9. テーブル駆動テスト](#8-1-9-テーブル駆動テスト)
      - [8-1-10. サブテストとテーブル駆動テスト](#8-1-10-サブテストとテーブル駆動テスト)
      - [8-1-11. テストヘルパー](#8-1-11-テストヘルパー)
      - [8-1-12. テストケースを簡潔に書く](#8-1-12-テストケースを簡潔に書く)
      - [8-1-13. coverprofile](#8-1-13-coverprofile)
  - [8-2. テスタビリティ](#8-2-テスタビリティ)
      - [8-2-1. テスタブルなコードと抽象化](#8-2-1-テスタブルなコードと抽象化)
      - [8-2-2. インタフェースを使う](#8-2-2-インタフェースを使う)
      - [8-2-3. テストする部分だけ実装する](#8-2-3-テストする部分だけ実装する)
      - [8-2-4. 環境変数を使う](#8-2-4-環境変数を使う)
      - [8-2-5. テストデータを用意する](#8-2-5-テストデータを用意する)
      - [8-2-6. 非公開な機能を使ったテスト](#8-2-6-非公開な機能を使ったテスト)
