...
- [4. パッケージ](#4-パッケージ)
  - [4-1. パッケージ](#4-1-パッケージ)
      - [4-1-1. Goのプログラムの構成要素](#4-1-1-goのプログラムの構成要素)
      - [4-1-2. Goのプログラムの構成とパッケージ](#4-1-2-goのプログラムの構成とパッケージ)
      - [4-1-2. 複数のファイル](#4-1-2-複数のファイル)
      - [4-1-3. パッケージのインポート](#4-1-3-パッケージのインポート)
      - [4-1-4. パッケージ名のエイリアス](#4-1-4-パッケージ名のエイリアス)
      - [4-1-5. パッケージ外へのエクスポート](#4-1-5-パッケージ外へのエクスポート)
      - [4-1-6. GOPATH](#4-1-6-gopath)
      - [4-1-7. GOPATHの設定方法](#4-1-7-gopathの設定方法)
  - [4-2. パッケージ変数とスコープ](#4-2-パッケージ変数とスコープ)
      - [4-2-1. スコープ](#4-2-1-スコープ)
      - [4-2-2. ブロックスコープ](#4-2-2-ブロックスコープ)
      - [4-2-3. ファイルスコープ](#4-2-3-ファイルスコープ)
      - [4-2-4. パッケージスコープ](#4-2-4-パッケージスコープ)
      - [4-2-5. パッケージ変数](#4-2-5-パッケージ変数)
      - [4-2-6. ユニバーススコープ](#4-2-6-ユニバーススコープ)
  - [4-3. パッケージの初期化](#4-3-パッケージの初期化)
      - [4-3-1. パッケージの初期化](#4-3-1-パッケージの初期化)
      - [4-3-2. init関数](#4-3-2-init関数)
  - [4-4. ライブラリのバージョン管理](#4-4-ライブラリのバージョン管理)
      - [4-4-1. go getでライブラリを取得する](#4-4-1-go-getでライブラリを取得する)
      - [4-4-2. TRY ライブラリを取得してみよう](#4-4-2-try-ライブラリを取得してみよう)
      - [4-4-3. GoDocを生成する](#4-4-3-godocを生成する)
      - [4-4-4. Go Modules (vgo)](#4-4-4-go-modules-vgo)
      - [4-4-5. goinstallの登場](#4-4-5-goinstallの登場)
      - [4-4-6. go getの登場で解決したこと](#4-4-6-go-getの登場で解決したこと)
      - [4-4-7. go getで解決できなかったこと](#4-4-7-go-getで解決できなかったこと)
      - [4-4-8. ベンダリング](#4-4-8-ベンダリング)
      - [4-4-9. modules(vgo)までの流れ](#4-4-9-modulesvgoまでの流れ)
      - [4-4-10. Import Compatibility Rule](#4-4-10-import-compatibility-rule)
      - [4-4-10. Minimal Version Selection](#4-4-10-minimal-version-selection)
      - [4-4-11. モジュール](#4-4-11-モジュール)
      - [4-4-12. セマンティックバージョニング](#4-4-12-セマンティックバージョニング)
      - [4-4-13. modulesが普及するために必要なこと](#4-4-13-modulesが普及するために必要なこと)
