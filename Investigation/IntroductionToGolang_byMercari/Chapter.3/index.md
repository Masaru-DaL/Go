...
- [3. 関数と型](#3-関数と型)
	- [3-1. 型](#3-1-型)
			- [3-1-1. 組み込み型](#3-1-1-組み込み型)
			- [3-1-2. 型変換(型のキャスト)](#3-1-2-型変換型のキャスト)
			- [3-1-3. TRY 組み込み型(数値)](#3-1-3-try-組み込み型数値)
			- [3-1-4. TRY 組み込み型(真偽値)](#3-1-4-try-組み込み型真偽値)
			- [3-1-5. コンポジット型](#3-1-5-コンポジット型)
			- [3-1-6. コンポジット型のゼロ値](#3-1-6-コンポジット型のゼロ値)
			- [3-1-7. 型リテラル](#3-1-7-型リテラル)
			- [3-1-8. 構造体](#3-1-8-構造体)
			- [3-1-9. 構造体リテラル](#3-1-9-構造体リテラル)
			- [3-1-10. 文法で理解しよう](#3-1-10-文法で理解しよう)
			- [3-1-11. フィールドの参照](#3-1-11-フィールドの参照)
			- [3-1-12. 配列](#3-1-12-配列)
			- [3-1-13. 配列の初期化](#3-1-13-配列の初期化)
			- [3-1-14.　配列の操作](#3-1-14配列の操作)
			- [3-1-15. 配列とスライス](#3-1-15-配列とスライス)
			- [3-1-16. スライス](#3-1-16-スライス)
			- [3-1-17. スライスの初期化](#3-1-17-スライスの初期化)
			- [3-1-18. スライスと配列の関係](#3-1-18-スライスと配列の関係)
			- [3-1-19. スライスの操作](#3-1-19-スライスの操作)
			- [3-1-20. appendの挙動](#3-1-20-appendの挙動)
			- [3-1-21. 配列・スライスへのスライス演算](#3-1-21-配列スライスへのスライス演算)
			- [3-1-22. スライスの要素をfor文で取得する](#3-1-22-スライスの要素をfor文で取得する)
			- [3-1-23. Slice Tricks](#3-1-23-slice-tricks)
			- [3-1-24. x/exp/slicesパッケージ](#3-1-24-xexpslicesパッケージ)
			- [3-1-25. TRY スライスの利用](#3-1-25-try-スライスの利用)
			- [3-1-26. マップ](#3-1-26-マップ)
			- [3-1-27. マップの初期化](#3-1-27-マップの初期化)
			- [3-1-28. マップの操作](#3-1-28-マップの操作)
			- [3-1-29. マップの要素をfor文で取得する](#3-1-29-マップの要素をfor文で取得する)
			- [3-1-30. x/exp/maps　パッケージ](#3-1-30-xexpmapsパッケージ)
			- [3-1-31. コンポジット型を要素にする](#3-1-31-コンポジット型を要素にする)
			- [3-1-33. Underlying type](#3-1-33-underlying-type)
			- [3-1-34. ユーザ定義型の特徴](#3-1-34-ユーザ定義型の特徴)
			- [3-1-35. TRY ユーザ定義型の利用](#3-1-35-try-ユーザ定義型の利用)
			- [3-1-36. 型エイリアス(Go1.9以上)](#3-1-36-型エイリアスgo19以上)
	- [3-2. 関数](#3-2-関数)
			- [3-2-1. 関数呼び出し](#3-2-1-関数呼び出し)
			- [3-2-2. 組み込み関数](#3-2-2-組み込み関数)
			- [3-2-3. 関数の定義](#3-2-3-関数の定義)
			- [3-2-4. 多値の受け取り方](#3-2-4-多値の受け取り方)
			- [3-2-5. 関数の定義](#3-2-5-関数の定義)
			- [3-2-6. 値の入れ替え](#3-2-6-値の入れ替え)
			- [3-2-7. 無名関数](#3-2-7-無名関数)
			- [3-2-8. 関数型](#3-2-8-関数型)
			- [3-2-9. クロージャと良くあるバグ](#3-2-9-クロージャと良くあるバグ)
			- [3-2-10. 値のコピー](#3-2-10-値のコピー)
			- [3-2-11. ポインタ](#3-2-11-ポインタ)
			- [3-2-12. 内部でポインタを使っているデータ型](#3-2-12-内部でポインタを使っているデータ型)
			- [3-2-13. TRY 奇数偶数判定関数](#3-2-13-try-奇数偶数判定関数)
			- [3-2-14. TRY 複数戻り値の利用](#3-2-14-try-複数戻り値の利用)
			- [3-2-15. TRY ポインタ](#3-2-15-try-ポインタ)
	- [3-3. メソッド](#3-3-メソッド)
			- [3-3-1. メソッド](#3-3-1-メソッド)
			- [3-3-2. レシーバ](#3-3-2-レシーバ)
			- [3-3-3. ポインタ型のメソッドリスト](#3-3-3-ポインタ型のメソッドリスト)
			- [3-3-4. レシーバに出来る型](#3-3-4-レシーバに出来る型)
			- [3-3-5. TRY レシーバに変更を与える](#3-3-5-try-レシーバに変更を与える)
			- [3-3-6. メソッド値](#3-3-6-メソッド値)
			- [3-3-7. メソッド式](#3-3-7-メソッド式)
