...
PUTには以下の2つの役割がある。
1. 新規リソースの作成
大きな点は**URIをクライアントが指定する点**です。
例えば、`/weblogs/myblog/entries/1`(URI)に対してPUTリクエストを送信すると、このURIで新しいリソースが作成されます。(URIで指定したリソースが存在しない場合)

リソースの作成は`POST`じゃないの？という疑問が湧くのですが、`PUT`は冪等性で、呼び出し回数が1回でも複数回でも同じ効果になる(副作用がない)のに対し、`POST`は連続して同じものを実行すると注文を複数回渡してしまうなどの追加の影響が発生する可能性があるようです。
また、"成功時のレスポンスの本文", "キャッシュ", "HTMLフォームでの利用"などにも差があります。

1. 既存リソースの更新
新しいリソースの表現をリクエストのエンティティボディに含めて指定したURIにPUTリクエストを送信することで、指定した既存リソースがリクエストのエンティティボディの内容で更新(上書き)されます。

#### 4-4-3. POST -> リソースの作成
POSTには以下の3つの役割がある。
1. 親リソースに対して子リソースの作成
**POSTの代表的な機能**。
主に掲示板・ブログ記事などの投稿に使われる。

例えば、ブログリソース(親リソース)を`/webblogs/myblog`と提供している場合、個々のブログエントリ(子リソース)を、`/weblogs/myblog/entries/1`として提供することができます。

リソース間に親子関係(従属関係)が存在する場合、親リソースに対してPOSTリクエストを送信することで子リソースを作成することができる。(上の例で言うと、`/webblogs/myblog`に対してPOSTリクエストを送信する。)

**作成されたリソースのURIはサーバ側で決められる。**

2. 既存リソースへの状態の追加
1の子リソースの作成ではなく、**状態の追加**である。
ちょっと良くわからんのでざっくりとだけですが、
`/log`に対してPOSTリクエストを送信すると、既存リソースにPOSTした状態を追加しているということです。

3. オーバーロードPOST
[RESTful Webサービス](https://www.oreilly.co.jp/books/9784873113531/)では"オーバーロードPOST"と命名されています。

他のメソッドでは対応できない処理の実行を行います。

> Webサービスによっては、統一インターフェースにないメソッドを提供するための道具として POST が使用されるケースがある。例えば、ユーザ管理 API のパスワードリセット機能の URI を `/users/password/reset` とし、必要なパラメータを POST するような使われ方である。この場合、リクエストに用いられている実質的なメソッドは `URI` に含まれている `reset` である。このような POST の使用方法を、本書ではオーバーロード POST と呼んでいる。

しかし、オーバーロードPOSTは統一インターフェースを破壊するため、使用は避けるべき。
オーバーロードPOSTを使用して良い時は、統一インターフェースを破壊する以外に実現方法がないような、複雑なシステムを構築する場合に限ります。

- POSTとPUTの使い分け
特別な理由がない限りは、リソースの作成はPOSTで行い、URIもサーバ側で決定するという設計が望ましいです。
"Webを支える技術"書籍より.
...
URIで指定したリソースのメタデータを取得するために使用される。
例えば:
- リソースのサイズ
- 更新日時
- など
...
URIで指定したリソースがサポートするHTTPメソッドの一覧を取得するために使用される。
`Allow: GET, HEAD`という形でレスポンスヘッダーに`Allowヘッダー`が含まれるので、そこを見ることでサポートされているメソッドを確認することができる。

## 5. REST API まとめ
1. RESTとRESTfulは広義で同じ事を言っていると捉える。
2. RESTとは設計思想である。
3. ROA(リソース指向アーキテクチャ)の実装を行う事でREST APIを実現してくれると思って良い
4. ROAの4つの概念を理解し、ROAの4つの特性に沿った実装を行う

結果、"**シンプルで効率的な開発が可能**"になります。

一方でRESTは設計思想であるがため、開発者によって記述方法がバラバラになりやすい事や、URIの適切な運用が面倒な点もあることも留意しておく必要があります。
