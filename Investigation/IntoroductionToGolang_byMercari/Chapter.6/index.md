...
- [6. 抽象化](#6-抽象化)
	- [6-1. インタフェース](#6-1-インタフェース)
			- [6-1-1. インタフェースと抽象化](#6-1-1-インタフェースと抽象化)
			- [6-1-2. インタフェース](#6-1-2-インタフェース)
			- [6-1-3. interface{}](#6-1-3-interface)
			- [6-1-4. 関数にインタフェースを実装させる](#6-1-4-関数にインタフェースを実装させる)
			- [6-1-5. スライスとインタフェース](#6-1-5-スライスとインタフェース)
			- [6-1-6. インタフェースの実装チェック](#6-1-6-インタフェースの実装チェック)
			- [6-1-7. 型アサーション](#6-1-7-型アサーション)
			- [6-1-8. 型スイッチ](#6-1-8-型スイッチ)
			- [6-1-9. インタフェースの設計](#6-1-9-インタフェースの設計)
			- [6-1-10. io.Readerとio.Writer](#6-1-10-ioreaderとiowriter)
			- [6-1-11. TRY インタフェースを作ろう](#6-1-11-try-インタフェースを作ろう)
	- [6-2. 埋め込みとインタフェース](#6-2-埋め込みとインタフェース)
			- [6-2-1. 構造体の埋め込み](#6-2-1-構造体の埋め込み)
			- [6-2-2. 埋め込みとフィールド](#6-2-2-埋め込みとフィールド)
			- [6-2-3. 埋め込みの特徴](#6-2-3-埋め込みの特徴)
			- [6-2-4. インタフェースと埋め込み](#6-2-4-インタフェースと埋め込み)
			- [6-2-5. インタフェースの埋め込み](#6-2-5-インタフェースの埋め込み)
